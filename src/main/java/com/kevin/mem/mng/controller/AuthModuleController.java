...
public class AuthModuleController extends BaseController {
    ...
    @ApiOperation("刪除接口")
    @GetMapping("/deleteAuthModule")
    @OperatorLog(description = "/deleteAuthModule")
    public BaseResponse deleteAuthModule(Long id, @RequestHeader(required = false) @NotBlank(message = "用户名不能为空") String userName){
